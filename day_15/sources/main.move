...
module challenge::day_15 {
    ...
    fun plant(counters: &mut FarmCounters, plotId: u8) {
        assert!((plotId >= 0 && plotId <= (MAX_PLOTS as u8)), E_INVALID_PLOT_ID); // 1 ile 20 arasi sinir kontrolu

        let len = vector::length(&counters.plots);
        assert!(len < MAX_PLOTS, E_PLOT_LIMIT_EXCEEDED); // maximum plot sinirlamasi
    ...
    fun harvest(counters: &mut FarmCounters, plotId: u8) {
        ...
        while (i < len) {
            ...
            if (*existing_plot == plotId) {
                found_index = i; // Eşleşme bulursak indeksi kaydediyoruz
            };
            i = i + 1;
        };
